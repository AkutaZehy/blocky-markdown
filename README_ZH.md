...
- 🖱 **悬停控件** - 块按钮悬停时浮现（触屏设备常显）
- 🚀 **无需构建** - 纯 HTML、CSS 和 JavaScript，以原生 ES modules 加载
...
- **重排**：拖拽到目标块前/后；顶部/上/下/底按钮或点击 **#N** 序号设置位置，均按数组顺序重新插入。可移到 **Cache**，也可从缓存恢复到工作区末尾。
- **编辑模式**：单击块进入行内编辑；**Edit/Done** 提供专注模式。底部 Tips 显示上下文提示。
- **预览**：使用 **Preview/Edit** 开关渲染当前工作区；每个块的预览区也会就地渲染自身 Markdown（无 Marked 时回退为纯文本）。
...
- **HTML5** - 结构
- **CSS3** - 带 CSS 变量的双主题
- **JavaScript ES Modules** - 应用逻辑，原生加载、免构建
- **Marked.js** - Markdown 解析（带备用方案）
- **DOMPurify** - 预览 HTML 消毒
- **Mermaid.js** - 预览中的图表渲染（按需懒加载）
...
5. **块编辑丢失格式细节**
   - 表格编辑器用单行 `<input>` 重建单元格（`TableEditorManager.confirm`），会保留列对齐标记，但不支持单元格内多行内容。
   - 代码块语言解析只接受 `\w+` 形式的语言标签（`BlockFactory` 的正则不涵盖 `c#`、`c++` 等），这些语言会被截断。

## 开发

Markdown 解析逻辑的单元测试使用 Node.js 内置测试 runner（Node 18+，无依赖）：
